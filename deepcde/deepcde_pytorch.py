...
class cde_loss(nn.Module):
    ...
    def forward(self, beta, z_basis, shrink_factor):
        ...
        first_terms = (beta * z_basis.view(-1, 1).expand_as(beta)).sum(dim=1) + 1.0
        loss = ((complexity_terms - 2 * first_terms) / shrink_factor).float().mean()
        return loss
...
class cde_nll_loss(nn.Module):
    ...
    def forward(self, beta, z_basis, shrink_factor):
        first_terms = (beta * z_basis.view(-1, 1).expand_as(beta)).sum(dim=1) + 1.0
        loss = ((- 1 * first_terms) / shrink_factor).float().mean()
        return loss
